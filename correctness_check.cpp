...
#pragma omp parallel for private(k,outliers, idx_remove, Best_idx_remove,src_random, dst_random,H) schedule(static)
for(k = 0; k< 5000; k++){
    ...
    #pragma omp critical
    for(j = 0; j<3; j++){
      cout <<"src_vec "<<src_vec.back().x<<" "<<src_vec.back().y<<" "<<src_vec.back().z<<endl; 
      src_random.push_back(src_vec.back());
      cout << "src_random "<<src_random[j].x<<src_random[j].y<<src_random[j].z<<endl; 
      src_vec.pop_back();
      dst_random.push_back(dst_vec.back());
      cout <<"dst_random"<<dst_random[j].x<<dst_random[j].y<<dst_random[j].z<<endl; 
      dst_vec.pop_back();
    }
